...
2. **用户可干预**：用户可以在第一步后确认或修改分析结果，确保理解正确
3. **更好的推理**：第一步的分析结果作为上下文传递给第二步，帮助 LLM 更准确地生成操作
...
| 操作类型     | 说明       | 输入           | 输出     |
| ------------ | ---------- | -------------- | -------- |
| `aggregate`  | 整列聚合   | 一列或多列数据 | 单个值   |
| `add_column` | 新增计算列 | 每行的单元格   | 新的一列 |
| `compute`    | 标量运算   | 已有的变量     | 单个值   |
